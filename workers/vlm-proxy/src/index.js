...
function normalizeRegion(item, width, height) {
  ...
  const x1 = clampNumber(item?.x1, 0, width, 0);
  const y1 = clampNumber(item?.y1, 0, height, 0);
  const x2 = clampNumber(item?.x2, 0, width, width);
  const y2 = clampNumber(item?.y2, 0, height, height);
  return {
    label,
...
function buildPrompt(width, height, context) {
  return [
    ...
    `图片尺寸为 ${width}x${height}，证据区域坐标必须使用该像素坐标系。`,
    ...
    "输出格式：",
    '{"predictions":[{"event":"overflow","score":0.88}],"regions":[{"label":"overflow_area","score":0.84,"x1":10,"y1":20,"x2":200,"y2":220}],"rationale":"一句话说明核心视觉依据"}',
