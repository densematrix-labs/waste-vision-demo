...
function buildPrompt(width, height, context) {
  const lines = [
    ...
    "必须完整检查画面所有区域；只有在真实图像中能清楚看到独立垃圾袋、纸箱、泡沫箱或大件杂物堆放主体时，才输出对应 region。",
    "现场上下文、画面叠加字幕、时间戳、水印、截图 UI、桶身印刷文字和桶盖标识都不能作为证据；不得根据任何文字线索推断、补充或编造 region。",
    "每个区域只框一个堆放主体或堆放团块，避免重复框同一堆。",
    "证据区域必须使用紧贴物体外轮廓的最小外接矩形，只框可见垃圾袋、纸箱、泡沫箱或大件杂物本体；边缘留白尽量控制在物体宽高的 3% 以内。",
    "不要框整片地面，不要框垃圾桶本体、桶盖、桶身广告/标识、道路、墙面、阴影、污渍、反光、水印或背景；如果堆放物旁边有大量空地，必须收紧坐标。",
    "如果疑似目标主要位于垃圾桶桶盖上、桶身上或半透明白色中文水印/字幕覆盖区域内，且不能清楚看到桶外地面上的实体堆放物，必须删除该 region。",
    "如果多个堆放物彼此接触或明显属于同一堆，合并成一个紧凑框；如果相隔较远，分成多个框。",
    "多个框只在每个区域都能独立触发堆放复核时输出；不确定、低置信、太小或只是孤立纸片、展开纸板、包装印刷的区域不要输出。",
    "输出前逐个复核 regions：如果框内主要内容不是桶外垃圾袋、纸箱、泡沫箱或大件杂物本体，必须删除该 region。",
